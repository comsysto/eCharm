...
from charging_stations_pipelines.pipelines.de.bna_crawler import (
...
def test_get_bna_data_downloads_file_with_correct_url(
    mock_getsize, mock_requests_get, mock_download_file, mock_beautiful_soup
):
    ...
    mock_download_file.assert_called_once_with(
        "https://some_ladesaeulenregister_url.xlsx",
...
def test_get_bna_data_logs_error_when_no_download_link_found(mock_beautiful_soup, mock_requests_get, caplog):
    ...
    with pytest.raises(
        bna_crawler.ExtractURLException,
        match="Failed to extract the download url from the website.",
    ):
...
@patch.object(charging_stations_pipelines.pipelines.de.bna_crawler, "download_file")
@patch.object(os.path, "getsize")
def test_get_bna_data_logs_file_size_after_download(
    mock_getsize,
    mock_download_file,
    mock_beautiful_soup,
    mock_requests_get,
    ...
    mock_download_file.assert_called_once_with(
        "tmp_data_path/ladesaeulenregister.xlsx",
