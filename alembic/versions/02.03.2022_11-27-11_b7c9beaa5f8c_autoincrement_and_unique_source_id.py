...
def upgrade():
    ...
    op.alter_column('address', 'station_id',
               existing_type=sa.INTEGER(),
               nullable=False)
    op.create_unique_constraint(None, 'address', ['station_id'])
    op.alter_column('charging', 'station_id',
               existing_type=sa.INTEGER(),
               nullable=False)
    op.create_unique_constraint(None, 'charging', ['station_id'])
    op.alter_column('stations', 'source_id',
               existing_type=sa.VARCHAR(),
               nullable=False)
    op.drop_index('ix_stations_source_id', table_name='stations')
    op.create_index(op.f('ix_stations_source_id'), 'stations', ['source_id'], unique=True)
