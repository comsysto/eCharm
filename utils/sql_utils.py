    ...
    for value in entry:
        if value is not None:
            try:
                ...
                )  # escape colon (binding parameter)
            except Exception:
    ...
    for chunk in chunks(list_obj=insert_dict, batch_size=batch):
        ...
        for col in column_names_list:
            excluded = '{excluded} "{col_name_left_side}"=EXCLUDED."{col_name_right_side}",'.format(
                excluded=excluded, col_name_left_side=col, col_name_right_side=col
            )
...
def insert_return_ids(
    ...
    insert_stmt_str = "{insert} RETURNING {sequencial_key}".format(
        insert=insert_stmt_str, sequencial_key=sequencial_key
    )
    insert_stmt_str = insert_stmt_str.replace("'null'", "null")
    with session_scope() as session:
