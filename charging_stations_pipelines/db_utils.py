...
def delete_all_merged_data(session: Session):
    ...
    session.execute(text(f'TRUNCATE TABLE {station.MergedStationSource.__tablename__} RESTART IDENTITY'))
    session.execute(delete(address.Address).where(address.Address.is_merged.is_(True)))
    session.execute(delete(charging.Charging).where(charging.Charging.is_merged.is_(True)))
